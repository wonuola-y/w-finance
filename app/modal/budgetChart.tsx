...
const BudgetChart = () => {
    return (
        <div className="bg-white p-5 md:p-8 rounded-xl w-full h-2/4 md:h-auto">
                ...
                <div className=" m-[a] w-full flex-col  py-6 pt-0">
                    {data.map((item) => (
                        <div key={item.id} className="flex  justify-between  text-sm py-4 border-b-[1px] border-b-gray-200" >
                            ...
                            <p className="text-gray-600 text-xs"> <span className="text-black font-semibold text-base mr-2">${item.spent.toFixed(2)}</span> of ${item.value.toFixed(2)}</p>
                        </div>
                    ))}
                </div>
            </div>
